...
async function loadReports() {
    ...
    style.textContent = `
        ...
        .report-section h3 { font-size: 1.1rem; font-weight: 600; margin-bottom: 12px; }
        .report-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px; }
        .report-stat { background: var(--bg-card); padding: 16px 20px; border-radius: var(--radius); border: 1px solid var(--border); }
        .report-stat .label { font-size: 0.8rem; color: var(--text-muted); }
        .report-stat .value { font-size: 1.4rem; font-weight: 700; }
        .report-table { width: 100%; border-collapse: collapse; font-size: 0.9rem; }
        .report-table th { text-align: left; padding: 8px 12px; border-bottom: 2px solid var(--border); font-weight: 600; color: var(--text-secondary); }
        .report-table td { padding: 8px 12px; border-bottom: 1px solid var(--border); }
        .report-table tr:hover { background: var(--primary-light); }
        .report-table .clickable { cursor: pointer; color: var(--primary); }
        .report-table .clickable:hover { text-decoration: underline; }
        .report-highlight { background: var(--bg-card); border-radius: var(--radius); padding: 16px 20px; border: 1px solid var(--border); margin-bottom: 12px; cursor: pointer; transition: all var(--transition); }
        .report-highlight:hover { box-shadow: var(--shadow); }
        .report-highlight .label { font-size: 0.8rem; color: var(--text-muted); }
        .report-highlight .value { font-size: 1.2rem; font-weight: 700; }
    `;
    ...
    try {
        ...
        let html = `
            ...
            <div class="report-grid" style="margin-bottom:24px;">
                <div class="report-stat" onclick="viewFilteredTransactions('income', '${dateFrom}', '${dateTo}')" style="cursor:pointer;">
                    <div class="label">Total Income</div>
                    <div class="value text-success">${formatCurrency(summary.income)}</div>
                </div>
                <div class="report-stat" onclick="viewFilteredTransactions('expense', '${dateFrom}', '${dateTo}')" style="cursor:pointer;">
            ...
            ${summary.allocations > 0 ? `
                <div class="report-stat" style="margin-bottom:16px;">
                    <div class="label">Savings Allocations</div>
                    <div class="value text-primary">${formatCurrency(summary.allocations)}</div>
                </div>
            ` : ''}
                    ...
                    <div class="report-grid">
                        ...
                        ${allTxns.length > 0 ? `
                            <div class="report-highlight">
                                ...
                                <div class="value">${formatCurrency(allTxns.reduce((s, t) => s + t.amount, 0) / allTxns.length)}</div>
                            </div>
                        ` : ''}
                    </div>
                </div>
            ...
            <div class="report-section">
                ...
                ` : '<div class="text-muted">No expenses to show</div>'}
            </div>
            ...
            <div class="report-section">
                ...
                ` : '<div class="text-muted">No income to show</div>'}
            </div>
        `;
...
async function viewFilteredTransactions(type, dateFrom, dateTo) {
    ...
    // We'll need to pass filter state
}
